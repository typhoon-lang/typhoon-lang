...
static void uring_submit(UringDriver* d, PendingReq* req) {
  ...
  sqe->fd = (int)req->fd; /* io_uring sqe->fd is int32; POSIX fd fits */
...
static void uring_close(ty_fd_t fd) { close((int)fd); }
...
static void kq_poll(KqDriver* d) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    int64_t result;
    if (req->is_write)
      result = (int64_t)write((int)req->fd, req->buf, req->len);
    else
      result = (int64_t)read((int)req->fd, req->buf, req->len);
    if (result < 0)
...
static void kq_close(ty_fd_t fd) { close((int)fd); }
...
static void iocp_close(ty_fd_t fd) {
...
void ty_io_driver_poll(void* driver) {
  if (!driver) return;
  IoDriver* d = (IoDriver*)driver;
#if defined(__linux__)
...
void ty_io_close(void* driver, ty_fd_t fd) {
  (void)driver;
#if defined(__linux__)
  uring_close(fd);
#elif defined(__APPLE__)
...
static void do_submit_or_sync(void* driver_ptr, SlabArena* arena, void* coro,
  ty_fd_t fd, uint8_t* buf, size_t len, int is_write) {
...
int64_t ty_io_read(void* driver, SlabArena* arena, void* coro,
  ty_fd_t fd, uint8_t* buf, size_t len) {
...
int64_t ty_io_write(void* driver, SlabArena* arena, void* coro,
  ty_fd_t fd, const uint8_t* buf, size_t len) {
